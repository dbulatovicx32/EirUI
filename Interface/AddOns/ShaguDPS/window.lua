...
local function CreateBar(parent, i, background)
  ...
  parent.bars[i].textLeft:SetFont("Interface\\AddOns\\pfUI-fonts\\fonts\\AccidentalPresidency.ttf", 11)
  ...
  parent.bars[i].textLeft:SetPoint("BOTTOMRIGHT", parent.bars[i], "BOTTOMRIGHT", -5, 1)

  parent.bars[i].textRight = parent.bars[i].textRight or parent.bars[i]:CreateFontString("Status", "OVERLAY", "GameFontNormal")
  parent.bars[i].textRight:SetFont("Interface\\AddOns\\pfUI-fonts\\fonts\\AccidentalPresidency.ttf", 11)
  ...
  parent.bars[i].textRight:SetPoint("BOTTOMRIGHT", parent.bars[i], "BOTTOMRIGHT", -5, 1)
...
window.btnMode = CreateFrame("Button", "ShaguDPSOverall", window)
...
window.btnAnnounce = CreateFrame("Button", "ShaguDPSAnnounce", window)
...
window.Refresh = function(force, report)
  ...
  for name, unitdata in spairs(segment, sort) do
    ...
    if bar >= 1 and bar <= config.bars then
      ...
      window.bars[bar]:SetStatusBarColor(0.23, 0.23, 0.23)
      window.bars[bar].textLeft:SetTextColor(values.color.r, values.color.g, values.color.b)
      window.bars[bar].textRight:SetTextColor(values.color.r, values.color.g, values.color.b)
