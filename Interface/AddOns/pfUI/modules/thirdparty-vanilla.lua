pfUI:RegisterModule("thirdparty-vanilla", "vanilla", function()
  ...
  HookAddonOrVariable("TWThreat", function()
    local docktable = { "twt", "TODO", "TWTMain",
      ...
      function() -- dual
        TWTMain:ClearAllPoints()
        TWTMain:SetPoint("TOPLEFT", pfUI.chat.right, "TOPLEFT", 0, -9)
    ...
    if C.thirdparty.twt.skin == "1" then
      ...
      for i, button in pairs(buttons) do
        ...
        local p,rt,rp,xo,yo = b:GetPoint()
        if not b.pfSet then
          b:SetPoint(p,rt,rp,xo - 5,yo + 2.5)
          b.pfSet = true
        end
      end

      -- buttons
      local color = { r = 1, g = 1, b = 1, c = "|cffffffff" }

      TWTMainSettingsButton:SetText(color.c .."O")
      TWTMainLockButton:SetText(color.c .."L")
      TWTMainCloseButton:SetText(color.c .."X")
